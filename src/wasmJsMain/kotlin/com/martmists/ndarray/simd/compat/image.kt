...
fun ImageData.toArray(): F64ImageArray {
    return F64Array.Companion.of(DoubleArray(data.length) { data[it].toInt() / 255.0 }).reshape(width, height, 4).image
}

fun F64ImageArray.toImageData(): ImageData {
    val doubles = flatten().toDoubleArray()
    val arr = Array(doubles.size) { (doubles[it] * 255.0).roundToInt().coerceIn(0, 255).toJsNumber() }.toJsArray()
